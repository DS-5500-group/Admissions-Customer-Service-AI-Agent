...
async def transfer_to_human(user_input: str, call: CallState, websocket: WebSocket):
    ...
    await websocket.send_json({"type": "text", "token": "I will transfer you to a human agent. Please stay on the line.", "last": True})
...
async def end_call_set_condition(user_input: str, call: CallState, websocket: WebSocket):
    ...
    if not call.email: #If email is not already set, the program will ask one last time before ending the call.
        await websocket.send_json({"type": "text","token": "Before we hang up, this is the last chance to get the transcript emailed to you. Just say 'email' or 'send email'.", "last": True})
        while True:
            ...
            if "email" in responseObtained.lower() or "send email" in responseObtained.lower():
...
async def websocket_endpoint(websocket: WebSocket, caller_number: str, callSid: str):
    ...
    if msg.get("type") == "setup":
        ...
        callSession.transcript.append({"speaker": "Agent", "text": "What language do you prefer? The available options are English, Spanish, and Chinese. The change will take effect after the email question."})
        await websocket.send_json({"type": "text", "token": "What language do you prefer? The available options are English, Spanish, and Chinese. The change will take effect after the email question.", "last": True})
