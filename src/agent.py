...
async def end_call_set_condition(user_input: str, call: CallState, websocket: WebSocket):
    ...
    email = call.email
    if not call.email:
        await websocket.send_json({"type": "text","token": "Before we hang up, this is the second chance to get the transcript emailed to you. Just say 'email' or 'send email'.", "last": True})
        while True:
    ...
    if email:
        ...
        try:
            await asyncio.to_thread(email_transcript, email, call, call.language)
