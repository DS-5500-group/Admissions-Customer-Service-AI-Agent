...
def intro():
    ...
    return Response(content=str(respond), media_type="application/xml")


# #Back and Forth conversation between the user and the bot
@app.post("/bothumaninteraction")
def conversation(SpeechResult: str = Form(...)):
    print("Caller Statement: ", SpeechResult)
    userStatement = SpeechResult.lower()
    ...
    if any(word in ["no", 'not','nope','none'] for word in wordDivided):
        ...
        answer.hangup()
        return Response(content=str(answer), media_type="application/xml")
